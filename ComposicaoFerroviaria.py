...
class ComposicaoFerroviaria(Deque, Persistente):
    ...
    def carregar(self):
        # Carrega a composição do arquivo .pkl
    ...
    def criar_composicao_padrao(self): # Cria uma composição padrão conforme anexo da atividade

        self._data = [None] * self._N # Reinicializa a Composição Ferroviária (Deque).
        self._size = 0
        self._front = 0
        self._top = 0
    ...
    def inserir_vagao_frente(self, vagao):
        # Insere vagão na frente e salva.
        self.addFirst(vagao)
        self.salvar()

    def inserir_vagao_final(self, vagao):
        # Insere vagão no final e salva.
        self.addLast(vagao)
        self.salvar()

    def remover_vagao_inicio(self):
        # Remove vagão do início e salva.
        if not self.isEmpty():
    ...
    def calcular_comprimento_total(self):
        ...
        # Adiciona 2m entre cada vagão (tamanho - 1vagao * 2)
        espacos_entre_vagoes = (self.getSize() - 1) * 2
        return comprimento_total + espacos_entre_vagoes

    def calcular_peso_total(self):# Calcula o peso total da composição
        peso_total = 0
        for vagao in self._data:
            if vagao is not None:
    ...
    def contar_passageiros(self):# Conta o numero total de passageiros na composição.
        total_passageiros = 0
        for vagao in self._data:
    ...
    def contar_carga_total(self):# Contabiliza a carga transportada pela composição.
        total_carga = 0
        for vagao in self._data:
    ...
    def verificar_potencia(self):# Verifica se a potência das locomotivas é suficiente.
        potencia_total = 0
